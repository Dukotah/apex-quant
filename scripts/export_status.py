...
def _build_equity_curve(equities: Sequence[float],
                        timestamps: Sequence[str]) -> List[Dict[str, Any]]:
    """
...
def _build_positions(portfolio: Portfolio,
...
def build_status(
    *,
    ...
    return {
        ...
        "alerts": _build_alerts(now=now, halted=halted, halt_reason=halt_reason,
                                drift=drift, extra=extra_alerts),
    }
...
def _portfolio_from_row(row: Any, initial_capital: Decimal) -> Portfolio:
    ...
    for ticker, p in raw.items():
        ...
        portfolio.on_fill(FillEvent(
            ...
            commission=_ZERO, slippage=_ZERO,
            order_id="export", broker_order_id="export",
            timestamp=ts, is_paper=True,
        ))
        # Mark to the persisted current price so equity matches the recorded run.
        if cur != avg:
            portfolio.on_market(MarketEvent(bar=Bar(
                symbol=symbol, timestamp=ts,
                open=cur, high=cur, low=cur, close=cur, volume=_ZERO,
            )))
    return portfolio
...
def build_status_from_store(
    ...
    *,
    ...
    if not rows:
        portfolio = Portfolio(initial_capital)
        return build_status(
            now=now, mode=mode, portfolio=portfolio,
            equity_history=[], timestamp_history=[],
            validated_sharpe=validated_sharpe, gauntlet_reports=gauntlet_reports,
        )
    ...
    drift = None
    try:
        mon = DriftMonitor(DEPLOYED_STRATEGY_ID, validated_sharpe=validated_sharpe, window=GATE_DAYS)
        for eq in equities:
...
def main(argv: Optional[Sequence[str]] = None) -> int:  # pragma: no cover - thin I/O wiring
    ...
    parser.add_argument("--state", default=str(DEFAULT_STATE_PATH), help="path to the run_once state DB")
    ...
    parser.add_argument("--capital", default="100000", help="initial capital fallback for reconstruction")
