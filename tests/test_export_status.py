...
TOP_LEVEL_KEYS = {
    "generatedAt", "mode", "halted", "haltReason", "account", "equityCurve",
    "positions", "trades", "strategies", "paperGate", "gauntlet", "alerts",
}
ACCOUNT_KEYS = {
    "equity", "cash", "buyingPower", "peakEquity", "currentDrawdownPct",
    "dailyStartEquity", "dayPnl", "dayPnlPct",
}
...
def _portfolio_spy_100_at_110() -> Portfolio:
    ...
    p.on_fill(FillEvent(symbol=SPY, side=OrderSide.BUY, quantity=Decimal("100"),
                        fill_price=Decimal("100"), commission=Decimal("0"),
                        order_id="o1", broker_order_id="b1", timestamp=NOW))
    p.on_market(MarketEvent(bar=Bar(symbol=SPY, timestamp=NOW, open=Decimal("110"),
                                    high=Decimal("110"), low=Decimal("110"),
                                    close=Decimal("110"), volume=Decimal("0"))))
    return p


def _build(**kw):
    base = dict(now=NOW, mode="paper", portfolio=_portfolio_spy_100_at_110(),
...
def test_equity_curve_drawdown_fraction():
    ...
    snap = _build(equity_history=[100.0, 110.0, 99.0],
                  timestamp_history=["2024-06-01T00:00:00+00:00",
...
def test_drawdown_when_below_peak():
    ...
    p.on_fill(FillEvent(symbol=SPY, side=OrderSide.BUY, quantity=Decimal("100"),
                        fill_price=Decimal("100"), commission=Decimal("0"),
                        order_id="o", broker_order_id="b", timestamp=NOW))
    # Mark down to 0 value would be extreme; instead drop SPY to 0 net -> equity 90000.
    p.on_market(MarketEvent(bar=Bar(symbol=SPY, timestamp=NOW, open=Decimal("0.01"),
                                    high=Decimal("0.01"), low=Decimal("0.01"),
                                    close=Decimal("0.01"), volume=Decimal("0"))))
    # peak was 100000 at init; equity now 90000 + 100*0.01 = 90001.
    snap = build_status(now=NOW, mode="paper", portfolio=p,
                        equity_history=[100000.0, 90001.0],
                        timestamp_history=["2024-06-02T00:00:00+00:00",
                                           "2024-06-03T00:00:00+00:00"])
...
def test_paper_gate_fields_and_fraction_semantics():
    snap = _build()
    pg = snap["paperGate"]
    assert set(pg.keys()) == {"startDate", "daysElapsed", "daysRequired",
                              "rollingSharpe", "targetSharpe", "withinBacktestBand"}
...
def test_trades_normalized_and_optional_pnl():
    trades = [
        {"id": "t1", "timestamp": NOW, "ticker": "SPY", "side": "buy",
         "quantity": 10, "price": 100, "notional": 1000, "commission": 0,
         "strategyId": "multi_asset_trend", "reason": "entry"},
        {"id": "t2", "timestamp": NOW, "ticker": "SPY", "side": "sell",
         "quantity": 10, "price": 110, "notional": 1100, "commission": 0,
         "strategyId": "multi_asset_trend", "reason": "exit", "pnl": 100},
    ]
...
def test_build_from_store_round_trips(tmp_path):
    ...
    for i, eq in enumerate([100000, 100500, 101000]):
        store.save_run(
            RunReport(timestamp=base.replace(day=i + 1), mode="paper",
                      equity=float(eq), num_positions=1, orders_submitted=1),
