...
def test_cross_asset_value_tie_break_is_deterministic():
    s1 = CrossAssetValueStrategy(
        "s", [A, B, C], value_period=4, skip_recent=0, top_k=1, vol_window=2
    )
    s2 = CrossAssetValueStrategy(
        "s", [C, B, A], value_period=4, skip_recent=0, top_k=1, vol_window=2
    )
    for s in (s1, s2):
...
def test_value_momentum_tie_break_is_deterministic():
    vm1 = ValueMomentumStrategy(
        "s", [A, B, C], value_period=4, skip_recent=0, mom_period=2, top_k=1, vol_window=2
    )
    vm2 = ValueMomentumStrategy(
        "s", [C, B, A], value_period=4, skip_recent=0, mom_period=2, top_k=1, vol_window=2
    )
    for vm in (vm1, vm2):
