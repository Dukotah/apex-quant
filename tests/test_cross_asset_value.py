...
def test_warmup_no_signals():
    ...
    h = _Harness(CrossAssetValueStrategy("s", [A, B, C], value_period=8, skip_recent=2,
                                         top_k=1, vol_window=3))
    out = []
...
def test_buys_the_cheapest_laggard_not_the_winner():
    ...
    h = _Harness(CrossAssetValueStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                         top_k=1, vol_window=3))
    buys = []
...
def test_trend_filter_blocks_falling_value_trap():
    ...
    h = _Harness(CrossAssetValueStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                         top_k=2, use_trend_filter=True, trend_period=5,
                                         vol_window=3))
    out = []
    for d in range(18):
