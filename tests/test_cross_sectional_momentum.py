...
def test_warmup_no_signals():
    h = _Harness(
        CrossSectionalMomentumStrategy(
            "s", [A, B, C], mom_period=5, top_k=1, trend_period=5, vol_window=3
        )
    )
    out = []
    for _ in range(4):
...
def test_holds_top_k_leader_and_exits_laggard():
    ...
    h = _Harness(
        CrossSectionalMomentumStrategy(
            "s", [A, B, C], mom_period=6, top_k=1, trend_period=6, vol_window=4
        )
    )
...
def test_absolute_filter_blocks_falling_leader():
    ...
    h = _Harness(
        CrossSectionalMomentumStrategy(
            "s", [A, B, C], mom_period=5, top_k=2, trend_period=5, vol_window=3
        )
    )
    out = []
    for d in range(14):
...
def test_deterministic():
    def run():
        h = _Harness(
            CrossSectionalMomentumStrategy(
                "s", [A, B, C], mom_period=6, top_k=2, trend_period=6, vol_window=4
            )
        )
        sigs = []
        for d in range(16):
            sigs += [
                (s.symbol.ticker, s.side)
                for s in h.step_all({A: 10 + d * 2, B: 10 + d, C: 10 + d * 0.5})
            ]
        return sigs
