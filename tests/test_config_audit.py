...
def test_backtest_with_real_broker_is_info_only():
    issues = audit_config(
        _sane_facts(
            mode="backtest",
            broker="alpaca",
            alpaca_key_present=False,
            alpaca_secret_present=False,
            alpaca_key_len=0,
            alpaca_secret_len=0,
        )
    )
...
def test_position_exceeding_exposure_is_warning():
    issues = audit_config(
        _sane_facts(max_position_size_pct=Decimal("0.6"), max_total_exposure_pct=Decimal("0.5"))
    )
...
def test_issues_sorted_errors_first():
    issues = audit_config(_sane_facts(mode="live", broker="simulated", require_stop_loss=False))
    ...
    first_warn = min(
        (i for i, s in enumerate(severities) if s is Severity.WARNING), default=len(severities)
    )
