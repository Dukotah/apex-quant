...
class FakeBroker:
    ...
    def __init__(
        self, poll_states=None, existing=None, equity="100000", positions=None, fail_cancel=False
    ):
        self._states = list(
            poll_states or [FakeOrder(status="filled", filled_qty="10", filled_avg_price="100.50")]
        )
        self._i = 0
        self.submitted = []
        self.canceled = []
...
def _engine(broker, **kw):
    ...
    eng = AlpacaExecutionEngine(
        broker_client=broker, on_fill=fills.append, sleep=lambda _s: None, **kw
    )
    eng.connect()
    return eng, fills
...
def test_poll_pending_then_filled_emits_fill():
    ...
    eng = AlpacaExecutionEngine(
        ...
        on_fill=lambda f: sleeps.append("fill"),
...
def test_enum_valued_status_is_detected_as_terminal():
    ...
    sleeps = []
    eng = AlpacaExecutionEngine(
        broker_client=broker, on_fill=lambda f: None, sleep=sleeps.append, fill_poll_attempts=3
    )
    eng.connect()
    eng.submit_order(_order())
...
def test_factory_paper_alpaca_builds_paper_engine():
    cfg = AppConfig(
        mode=ExecutionMode.PAPER, broker=Broker.ALPACA, alpaca_key="k", alpaca_secret="s"
    )
