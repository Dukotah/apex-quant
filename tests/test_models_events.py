...
def test_bar_rejects_naive_timestamp():
    with pytest.raises(ValueError):
        Bar(symbol=_sym(), timestamp=datetime(2024, 1, 1),
...
def test_bar_rejects_high_below_low():
    with pytest.raises(ValueError):
        Bar(symbol=_sym(), timestamp=_ts(),
            open=Decimal("1"), high=Decimal("0.5"), low=Decimal("1"),
            close=Decimal("0.8"), volume=Decimal("100"))
...
def test_bar_rejects_negative_price():
    with pytest.raises(ValueError):
        Bar(symbol=_sym(), timestamp=_ts(),
            open=Decimal("-1"), high=Decimal("2"), low=Decimal("-1"),
            close=Decimal("1"), volume=Decimal("100"))
...
def test_bar_rejects_negative_volume():
    with pytest.raises(ValueError):
        Bar(symbol=_sym(), timestamp=_ts(),
            open=Decimal("1"), high=Decimal("2"), low=Decimal("1"),
            close=Decimal("1.5"), volume=Decimal("-5"))


def test_valid_bar_constructs():
    b = Bar(symbol=_sym(), timestamp=_ts(),
...
def test_position_unrealized_pnl_long():
    p = Position(symbol=_sym(), quantity=Decimal("10"),
...
def test_position_market_value_with_multiplier():
    ...
    p = Position(symbol=fut, quantity=Decimal("2"),
                 avg_entry_price=Decimal("5000"), current_price=Decimal("5010"))
...
def test_signal_event_valid():
    sig = SignalEvent(symbol=_sym(), side=OrderSide.BUY,
