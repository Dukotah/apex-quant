...
def test_warmup_no_signals():
    ...
    h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=8, skip_recent=2,
                                       mom_period=3, top_k=1, vol_window=3))
    out = []
...
def test_pure_value_weight_buys_the_cheap_laggard():
    ...
    h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                       mom_period=3, top_k=1, value_weight=Decimal("1.0"),
                                       vol_window=3))
    buys = []
    for d in range(16):
...
def test_pure_momentum_weight_buys_the_leader():
    ...
    h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                       mom_period=4, top_k=1, value_weight=Decimal("0.0"),
                                       vol_window=3))
    for d in range(16):
...
def test_combined_prefers_cheap_and_trending():
    ...
    h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=10, skip_recent=0,
                                       mom_period=3, top_k=1, vol_window=3))
...
def test_trend_filter_blocks_falling_pick():
    ...
    h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                       mom_period=3, top_k=2, use_trend_filter=True,
...
def test_deterministic():
    def run():
        h = _Harness(ValueMomentumStrategy("s", [A, B, C], value_period=8, skip_recent=0,
                                           mom_period=3, top_k=2, vol_window=4))
        sigs = []
        for d in range(18):
