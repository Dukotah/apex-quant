...
def _report(**kw) -> RunReport:
    base = dict(timestamp=datetime(2024, 1, 1, tzinfo=UTC), mode="paper",
                equity=100000.0, num_positions=0)
    base.update(kw)
    ...
    for i in range(rows):
        ...
        store.save_run(
            RunReport(timestamp=base + timedelta(days=i), mode=mode,
                      equity=100000.0 + i, num_positions=0,
