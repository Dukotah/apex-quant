...
def _held(symbol: Symbol, qty: Decimal, price: Decimal) -> Position:
    return Position(symbol=symbol, quantity=qty, avg_entry_price=price, current_price=price)
...
class TestATRStopValidation:
    ...
    def test_valid_atr_stop_passes(self):
        ...
        sig = FakeSignal(AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("190"), atr=Decimal("5"))
    ...
    def test_too_tight_atr_stop_rejected(self):
        ...
        sig = FakeSignal(AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("198"), atr=Decimal("5"))
    ...
    def test_too_wide_atr_stop_rejected(self):
        ...
        sig = FakeSignal(AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("180"), atr=Decimal("5"))
        assert rm.evaluate(sig, _port("AAPL", Decimal("200"))) is None

    def test_missing_atr_rejected_when_required(self):
        rm = RiskManager(self._cfg())
        sig = FakeSignal(AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("190"), atr=None)
        assert rm.evaluate(sig, _port("AAPL", Decimal("200"))) is None

    def test_nonpositive_atr_rejected(self):
        rm = RiskManager(self._cfg())
        sig = FakeSignal(AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("190"), atr=Decimal("0"))
        assert rm.evaluate(sig, _port("AAPL", Decimal("200"))) is None
...
class TestTrailingStopGate:
    def test_trailing_allowed_passes(self):
        ...
        sig = FakeSignal(
            AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("190"), trailing_stop=True
        )
        assert rm.evaluate(sig, _port("AAPL", Decimal("200"))) is not None
    ...
    def test_non_trailing_unaffected_when_disallowed(self):
        rm = RiskManager(_base_config(require_stop_loss=True, allow_trailing_stop=False))
        sig = FakeSignal(
            AAPL, OrderSide.BUY, suggested_stop_loss=Decimal("190"), trailing_stop=False
        )
        assert rm.evaluate(sig, _port("AAPL", Decimal("200"))) is not None
